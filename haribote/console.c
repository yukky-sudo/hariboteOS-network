...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
	if (strcmp(cmdline, "mem") == 0 && cons->sht != 0) {
		...
		cons_putstr0(cons, "dir : Listup Files. \n");
		cons_putstr0(cons, "start : Launch the program with new console.\n");
		cons_putstr0(cons, "ncst : Launch program without new console.\n");
		cons_putstr0(cons, "ip : View network infomation\n");
	} else if (cmdline[0] != 0) {
...
{
	char s[60];
	sprintf(s,"mac address: %02x:%02x:%02x:%02x:%02x:%02x",nic_mac_addr[0],nic_mac_addr[1],nic_mac_addr[2],nic_mac_addr[3],nic_mac_addr[4],nic_mac_addr[5]);
	cons_putstr0(cons, s);
	cons_newline(cons);
}
