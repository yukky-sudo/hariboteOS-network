...
void HariMain(void)
{
	...
	struct SHEET *sht_back, *sht_mouse, *sht_task, *sht_filer;
	...
	struct SHEET *sht = 0, *key_win, *sht2, *sht3;
	int *fat;
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	struct FILEINFO *f = (struct FILEINFO *) (ADR_DISKIMG + 0x002600);
	
	nic_init(); /* nicを初期化する(送受信なども起動する)*/
	
	sprintf(s,"nic_mac_addr: %02x:%02x:%02x:%02x:%02x:%02x",nic_mac_addr[0],nic_mac_addr[1],nic_mac_addr[2],nic_mac_addr[3],nic_mac_addr[4],nic_mac_addr[5]);
	//i = get_pci_conf_reg(NIC_BUS_NUM,NIC_DEV_NUM,NIC_FUNC_NUM,0);
	//sprintf(s,"version: Wings 95 NetworkTest");
	...
	for (i = 0; i < 224; i++) {
		...
		if (f[i].name[0] != 0xe5) {
			if ((f[i].type & 0x18) == 0) {
				sprintf(s, "filename.ext   %7d", f[i].size);
				for (j = 0; j < 8; j++) {
					s[j] = f[i].name[j];
				}
				s[ 9] = f[i].ext[0];
				s[10] = f[i].ext[1];
				s[11] = f[i].ext[2];
				putfonts8_asc(buf_back,binfo->scrnx,0,16+i*16,COL8_FFFFFF,s);
			}
		}
	}
            ...
            if (256 <= i && i <= 511) { // Keyboard
				...
				if (mouse_decode(&mdec, i - 512) != 0) {
					...
					if ((mdec.btn & 0x01) != 0) {
							...
							for (j = shtctl->top - 1; j > 0; j--) {
								...
								if (0 <= x && x < sht->bxsize && 0 <= y && y < sht->bysize) {
									if (sht->buf[y * sht->bxsize + x] != sht->col_inv) {
										...
										if (3 <= x && x < sht->bxsize - 3 && 3 <= y && y < 21 && j != 0) {
											mmx = mx;	/* ウィンドウ移動モードへ */
											mmy = my;
											mmx2 = sht->vx0;
											new_wy = sht->vy0;
						...
						{
							// 右クリック
							// 構想としては、マウスの上にコピーアイコン、下に貼り付けアイコン、右にその他の設定、左は自由領域
							// 描画用シートを使ってみる(作る。)
							sprintf(s,"Copy");
							putfonts8_asc(buf_back,binfo->scrnx,mx-32,my+16,COL8_FFFFFF,s);
						}
